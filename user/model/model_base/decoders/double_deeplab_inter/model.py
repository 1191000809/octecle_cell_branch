...
from user.model.model_base.base.double_module_inter import DoubleBranchModel_inter
...
from user.model.model_base.decoders.double_deeplab.model import DeepLabV3Plus_double
...
class DeepLabV3Plus_double_inter(DoubleBranchModel_inter):
...
class PytorchDeeplab_CellModel():
    ...
    def __init__(self, metadata=None):
        ...
        self.unet1 = DeepLabV3Plus_double_inter()
        self.unet2 = DeepLabV3Plus_double_inter() 
        self.unet3 = DeepLabV3Plus_double_inter()
        self.unet4 = DeepLabV3Plus_double()
        self.load_checkpoint()
       
        self.unet1 = self.unet1.to(self.device)
        self.unet2 = self.unet2.to(self.device)
        self.unet3 = self.unet3.to(self.device)
        self.unet4 = self.unet4.to(self.device)

        self.tta_model1 = tta.SegmentationTTAWrapper(self.unet1, tta.aliases.d4_transform(), merge_mode='mean')
        self.tta_model2 = tta.SegmentationTTAWrapper(self.unet2, tta.aliases.d4_transform(), merge_mode='mean')
        self.tta_model3 = tta.SegmentationTTAWrapper(self.unet3, tta.aliases.d4_transform(), merge_mode='mean')
        self.tta_model4 = tta.SegmentationTTAWrapper(self.unet4, tta.aliases.d4_transform(), merge_mode='mean')

        self.tta_model1.eval()
        self.tta_model2.eval()
        self.tta_model3.eval()
        self.tta_model4.eval()
        

    def load_checkpoint(self):

        # torch.save()包含了state_dict 加载自己的模型用下面的代码
        _path_to_checkpoint = 'user/model/model_base/decoders/double_deeplab_inter/checkpoints/epoch 1 double_deeplab_inter iou0.42156244133180043 valid_loss 0.44179956011673066.pth'
        print('loading model')
        checkpoint = torch.load(_path_to_checkpoint, map_location='cpu')
        self.unet1.load_state_dict(checkpoint['state_dict'])        
        print("Weights were successfully loaded!")

        _path_to_checkpoint = 'user/model/model_base/decoders/double_deeplab_inter/checkpoints/epoch 5 double_deeplab_inter iou0.4195719288115949 valid_loss 0.4421721431448104.pth'
        print('loading model')
        checkpoint = torch.load(_path_to_checkpoint, map_location='cpu')
        self.unet2.load_state_dict(checkpoint['state_dict'])        
        print("Weights were successfully loaded!")

        _path_to_checkpoint = 'user/model/model_base/decoders/double_deeplab_inter/checkpoints/epoch 12 double_deeplab_inter iou0.463195067773011 valid_loss 0.39857370704573003.pth'
        print('loading model')
        checkpoint = torch.load(_path_to_checkpoint, map_location='cpu')
        self.unet3.load_state_dict(checkpoint['state_dict'])        
        print("Weights were successfully loaded!")

        _path_to_checkpoint = 'user/model/model_base/decoders/double_deeplab/checkpoints/epoch 13 double_deeplab iou0.42204800942354964 valid_loss 0.44257487715798866.pth'
        print('loading model')
        checkpoint = torch.load(_path_to_checkpoint, map_location='cpu')
        self.unet4.load_state_dict(checkpoint['state_dict'])        
        print("Weights were successfully loaded!")
    ...
    def __call__(self, cell_patch, tissue_patch, roi_loc, roi_loc_ratio, pair_id):
        ...
        cell_logits1 = self.tta_model1(cell_patch, tissue_patch, roi_loc, roi_loc_ratio)
        cell_logits2 = self.tta_model2(cell_patch, tissue_patch, roi_loc, roi_loc_ratio)
        cell_logits3 = self.tta_model3(cell_patch, tissue_patch, roi_loc, roi_loc_ratio)
        cell_logits4 = self.tta_model4(cell_patch, tissue_patch, roi_loc, roi_loc_ratio)

        cell_logits = (cell_logits1+cell_logits2+cell_logits3+cell_logits4)/3
